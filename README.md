...
- `download/` — downloads for every platform, with a "for this machine" suggestion sniffed from the user agent (hidden on phones / when unsure). Static links to `releases/latest`; a small script upgrades them to direct versioned asset URLs via the GitHub API when reachable.
